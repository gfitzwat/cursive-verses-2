					...
					<form onSubmit={handleSubmit} className="space-y-5">
						...
						{TURNSTILE_SITE_KEY ? (
							<div className="flex justify-center">
								<div className="cf-turnstile" data-sitekey={TURNSTILE_SITE_KEY} data-callback="onTurnstileSuccess" data-expired-callback="onTurnstileExpired" data-error-callback="onTurnstileError" />
							</div>
						) : (
							<p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">Turnstile is not configured. Set VITE_TURNSTILE_SITE_KEY to enable contact form submissions.</p>
						)}

						{error && <p className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{error}</p>}

						<button type="submit" disabled={sending || !TURNSTILE_SITE_KEY || !turnstileToken} className="w-full rounded-lg bg-brand-teal hover:bg-brand-teal/85 text-white font-medium py-2.5 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed">
